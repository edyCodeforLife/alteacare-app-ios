...
class HistoryConsultationRepositoryImpl: HistoryConsultationRepository {
    ...
    func requestList(body: ListConsultationBody) -> Single<HistoryConsultation> {
        return Single.create { (observer) in
            self.listConsultationAPI
                .request(parameters: body.dictionary ?? [String: Any](), type: .history)
    ...
    private func outputTransformModel(_ response: ListConsultationResponse) -> Result<HistoryConsultation, HTTPError> {
        if let data = response.data, response.status {
            let model = data.map { (res) -> HistoryConsultationModel in
                ...
                return HistoryConsultationModel(id: "\(res.id ?? 0)",
                                                ...
                                                doctorImage: res.doctor?.photo?.url ?? "", transaction: transaction,
                                                patientFamilyMember: res.patient ?? res.parentUser,
                                                dateSchedule: res.schedule?.date)
            }
            let result = HistoryConsultation(isFullyLoaded: response.meta.page == response.meta.totalPage || model.isEmpty, model: model)
                return .success(result)
        } else if !response.status && ((response.data?.isEmpty) != nil) {
            let result = HistoryConsultation(isFullyLoaded: true, model: [])
            return .success(result)
        }
