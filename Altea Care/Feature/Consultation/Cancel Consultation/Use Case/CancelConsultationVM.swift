...
class CancelConsultationVM: BaseViewModel {
    
    private let repository: CancelConsultationRepository
    private let disposeBag = DisposeBag()
    private let stateRelay = BehaviorRelay<BasicUIState>(value: .loading)
    private let consultationListRelay = BehaviorRelay<[CancelConsultationModel]>(value: [])
    private let consultationMutableListRelay = BehaviorRelay<[CancelConsultationModel]>(value: [])
    ...
    struct Output {
        ...
        let consultationList: Driver<[CancelConsultationModel]>
        let consultationMutableList: Driver<[CancelConsultationModel]>
        let isFullyLoaded: Driver<Bool>
        let listMemberOutput: Driver<[MemberModel]>
    }
    
    init(repository: CancelConsultationRepository) {
